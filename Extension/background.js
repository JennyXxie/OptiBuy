// Shopping Price Tracker - Background Script with Integrated Database

// Shopping Database Class (Integrated)
...
class PriceTracker {
    ...
    async init() {
        ...
        // Listen for tab updates to check for price changes
    ...
    async handleMessage(request, sender, sendResponse) {
        try {
            switch (request.action) {
                case 'addProductTracking':
                    await this.addProductTracking(request.data);
                    sendResponse({ success: true });
                    break;
                case 'removeProductTracking':
                    await this.removeProductTracking(request.productId);
                    sendResponse({ success: true });
                    break;
                case 'getTrackedProducts':
                    const trackedProducts = await this.database.getTrackedProducts();
                    sendResponse({ products: Object.values(trackedProducts) });
                    break;
                case 'checkPrice':
                    await this.checkProductPrice(request.productId);
                    sendResponse({ success: true });
                    break;
                case 'productExtracted':
                    await this.handleProductExtraction(request.data);
                    sendResponse({ success: true });
                    break;
                default:
    ...
    handleTabUpdate(tabId, changeInfo, tab) {
        if (changeInfo.status === 'complete' && tab.url) {
            ...
            this.checkIfTrackedProductPage(tab);
        }
    }

    async checkIfTrackedProductPage(tab) {
        const url = tab.url.toLowerCase();
        const isEcommerceSite = url.includes('amazon.com') || 
    ...
    async addProductTracking(productData) {
        try {
            // First add the product to the database if it doesn't exist
            let productId = productData.id;
            if (!productId) {
                productId = await this.database.addProduct(productData);
            }

            // Add to tracking
            await this.database.addTrackedProduct(productId, {
                notificationThreshold: 0.1, // 10% price drop
                checkFrequency: 30 // 30 minutes
            });
            
            // Send notification
            this.showNotification('Product Added to Tracking', 
                `${productData.name} is now being tracked for price drops.`);
        } catch (error) {
            console.error('Error adding product tracking:', error);
        }
    }

    async removeProductTracking(productId) {
        try {
            await this.database.removeTrackedProduct(productId);
        } catch (error) {
            console.error('Error removing product tracking:', error);
        }
    }

    async handleProductExtraction(productData) {
        try {
            // Check if this product is already in our database
            const existingProducts = await this.database.getProducts();
            const existingProduct = Object.values(existingProducts).find(p => 
                p.url === productData.url || 
                (p.name === productData.name && p.source === productData.source)
            );
            ...
            if (existingProduct) {
                ...
                await this.database.addPriceHistory(existingProduct.id, productData.price, 'content_script');
            } else {
                // Add new product
                await this.database.addProduct(productData);
            }
        } catch (error) {
            console.error('Error handling product extraction:', error);
        }
    }

    startPriceMonitoring() {
        setInterval(() => {
            this.checkAllTrackedProducts();
        }, this.priceCheckInterval);
    }

    async checkAllTrackedProducts() {
        try {
            const trackedProducts = await this.database.getTrackedProducts();
            
            for (const [productId, trackingData] of Object.entries(trackedProducts)) {
                if (trackingData.is_active) {
            ...
            console.error('Error checking tracked products:', error);
        }
    }
    ...
    async checkProductPrice(productId) {
        try {
            ...
            const trackingData = await this.database.getTrackedProducts();
            const tracking = trackingData[productId];
            
            if (!product || !tracking) return;

            // Simulate price checking (in real implementation, this would call actual APIs)
            const currentPrice = await this.simulatePriceCheck(product);
            
            if (currentPrice && currentPrice !== product.currentPrice) {
                const priceChange = currentPrice - product.currentPrice;
                ...
                // Add price history
                await this.database.addPriceHistory(productId, currentPrice, 'price_monitor');
                
                // Update tracking data
                await this.database.updateTrackedProduct(productId, {
                    lastChecked: new Date().toISOString()
                });
                
                // Show notification if price dropped significantly
                if (priceChange < 0 && Math.abs(percentageChange) >= (tracking.notification_threshold * 100)) {
                    this.showPriceDropNotification(product, currentPrice, Math.abs(priceChange), Math.abs(percentageChange));
                }
            }
        } catch (error) {
    ...
    async simulatePriceCheck(product) {
        ...
        const basePrice = 200; // Base price for simulation
        const variation = (Math.random() - 0.5) * 50; // ±25 price variation
        const currentPrice = Math.max(50, basePrice + variation); // Minimum $50
        
        return Math.round(currentPrice * 100) / 100; // Round to 2 decimal places
    }

    showPriceDropNotification(product, newPrice, priceDrop, percentageDrop) {
        const notificationOptions = {
            type: 'basic',
            iconUrl: 'icons/icon48.png',
            title: '💰 Price Drop Alert!',
            message: `${product.name} dropped by $${priceDrop.toFixed(2)} (${percentageDrop}%) to $${newPrice}`,
            buttons: [
                { title: 'View Product' },
                { title: 'Stop Tracking' }
            ]
        ...
        chrome.notifications.create(`price_drop_${product.id}`, notificationOptions, (notificationId) => {
            ...
            chrome.notifications.onButtonClicked.addListener((clickedNotificationId, buttonIndex) => {
                if (clickedNotificationId === notificationId) {
                    if (buttonIndex === 0) {
                        ...
                        // Stop Tracking
                        this.removeProductTracking(product.id);
